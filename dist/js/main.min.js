...
var test = function(){
    ...
    var performTransition = function (articleEq, position) {

        if (!inScroll) {
            inScroll = true;

            var articleEq = articleEq - 1; /*Index starts counting from 0, eq from 1*/
            ...
            articles.eq(articleEq).addClass('active')
                .siblings().removeClass('active');

            setTimeout(function () {
                inScroll = false;
                $('.js-sidebar-article').eq(articleEq).addClass('active')
                    .siblings().removeClass('active');
            }, 300)
    ...
    var scrollToArticle = function (direction) {
        var article = defineArticle(articles);

        if (direction == 'up' && article.nextArticle.length) { /*вниз*/
            var position = article.nextArticle.offset().top;
            performTransition(article.nextArticle.index(), position);
        }

        if (direction == 'down' && article.prevArticle.length) { /*вверх*/
            var position = article.prevArticle.offset().top;
            performTransition(article.prevArticle.index(), position);
        }
        console.log(article.nextArticle.index(), position);
    }
    ...
    */
    ...
    $('.js-sidebar-article').on('click', function (e) {

        e.preventDefault();

        var title = $(e.target),
            titleEq = elem.index(),
            position = articles.eq(titleEq).offset().top;

        performTransition(titleEq + 1, ); /*Index starts counting from 0, eq from 1*/
